...
import type { BirthdayConfig, Video } from '../types';
...
const BirthdayWebsite: React.FC<BirthdayWebsiteProps> = ({ config, onStartOver }) => {
    ...
    // FIX: Correctly type the ref to hold an array of both Meshes and Groups.
    const objectsRef = useRef<(THREE.Mesh | THREE.Group)[]>([]);
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < 25; i++) {
            ...
            if (rand < 0.5) { // Heart
                ...
                giftGroup.add(boxMesh, ribbon1Mesh, ribbon2Mesh); obj = giftGroup; (obj as any).isGroup = true;
            }
        ...
        const animate = () => {
            ...
            if (starFieldRef.current) {
                // FIX: Removed incorrect type assertion to `number[]`. `array` is a TypedArray.
    ...
    const handleLaunchWish = () => {
      ...
      objectsRef.current.forEach(obj => {
          ...
          if ((obj as any).isGroup) {
              (obj as THREE.Group).children.forEach(child => {
                  if ((child as THREE.Mesh).material) gsap.to(((child as THREE.Mesh).material as THREE.Material), { opacity: 0, duration: 2.5, ease: 'power3.in' });
              });
          } else {
              gsap.to((obj as THREE.Mesh).material, { opacity: 0, duration: 2.5, ease: 'power3.in' });
          }
      });
            ...
            if ((obj as any).isGroup) {
              (obj as THREE.Group).children.forEach(child => {
                  if ((child as THREE.Mesh).material) {
                      gsap.killTweensOf(((child as THREE.Mesh).material as THREE.Material));
                      gsap.to(((child as THREE.Mesh).material as THREE.Material), { opacity: 1, duration: 1 });
                  }
              });
            } else {
               gsap.killTweensOf((obj as THREE.Mesh).material);
               gsap.to((obj as THREE.Mesh).material, { opacity: 1, duration: 1 });
            }
        });
        goToStep(1);
    };
    ...
    const VideoPlayer = ({ video }: { video: Video }) => {
        if (video.processed.type === 'data') {
            return <div className="aspect-video-container bg-black rounded-lg overflow-hidden shadow-lg"><video src={video.processed.url} controls className="w-full h-full" /></div>;
        }
        const embedUrl = video.processed.type === 'youtube'
            ? `https://www.youtube.com/embed/${video.processed.id}`
            : `https://drive.google.com/file/d/${video.processed.id}/preview`;
